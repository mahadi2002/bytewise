...
// Storage is Asia/Dhaka wall-clock time directly (02-SCHEMA.sql sets the
// migration session's time_zone to +06:00) — the process timezone matches
// so date()/strtotime() agree with what's in the DB, no conversion layer.
...
if (strlen((string) config('app.key')) !== 32 || strlen((string) config('app.hash_pepper')) !== 32) {
