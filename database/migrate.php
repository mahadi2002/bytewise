 ...
 *
 * Uses DB_MIGRATE_USERNAME / DB_MIGRATE_PASSWORD — the runtime app user has
 * no DDL rights (see §7 privilege probe), so a SQL-injection bug in the app
 * itself can never drop or alter a table.
 */
...
$cfg = config('db');
$mig = config('migrate_db');
$dsn = sprintf('mysql:host=%s;port=%d;charset=%s', $cfg['host'], $cfg['port'], $cfg['charset']);

$pdo = new PDO($dsn, (string) $mig['username'], (string) $mig['password'], [
 ...
 * string literal or a comment. Handles the seed file's `SELECT id INTO
 * @var FROM ...` MySQL user-variable pattern like any other statement.
 *
 * @return string[]
 */
